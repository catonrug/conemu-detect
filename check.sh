...
#git clone https://github.com/catonrug/conemu-detect.git && cd conemu-detect && chmod +x check.sh && ./check.sh
...
filelist=$(wget -qO- "$download" | sed "s/http/\nhttp/g;s/download/download\n/g" | grep "^http.*\/download$" | grep -v "PortableApps\|FarManager\|Alpha\|AutoUpdate" | sort | uniq | sed '$alast line')
